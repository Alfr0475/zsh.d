...
if [ -z "${LOADED_COMMON:-}" ]; then
    echo "${fg[red]}cannot start ZSH, \$LOADED_COMMON not set${reset_color}" 1>&2
    return 1
fi
...
if [ `uname` = "Linux" ]; then
    ...
    alias emacsclient='$(brew --prefix)/bin/emacsclient -s ~/.emacs.d/tmp/server/server -n'
